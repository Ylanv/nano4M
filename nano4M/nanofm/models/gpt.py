...
class GPT(nn.Module):
    ...
    def __init__(
        ...
        self.input_embedding = nn.Embedding(num_embeddings = vocab_size,embedding_dim = dim,padding_idx = padding_idx) 
    ...
    def forward_model(self, x: torch.LongTensor) -> torch.Tensor:
        ...
        assert embed.shape == (B,L,self.dim),f"Expected embedding shape ({B},{L}, {self.dim}), but got {embed.shape}"
        ...
        mask = torch.ones(L,L,dtype=torch.bool).tril(diagonal = 0).unsqueeze(0).to(device)
    ...
    def generate(
        ...
        for _ in range(self.max_seq_len - len(context)):
            ...
            samples, sampled_probs = sample_tokens(logits,temperature=temp,top_k=top_k,top_p=top_p)
