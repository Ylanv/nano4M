...
class MaskGIT(nn.Module):
    ...
    def __init__(
        ...
        self.input_embedding = nn.Embedding(num_embeddings=vocab_size,embedding_dim=dim) 
    ...
    def generate(
        ...
        for step, k in enumerate(schedule):
            ...
            logits = self.forward_model(seq,mask)
            assert logits.shape == (1,L,self.vocab_size), \
            ...
            masked_logits = logits[0,masked_indices] # Note : we can do tensor[tensor] to select index from a list (like in numpy) 
            
            assert masked_logits.dim() == 2, \
            ...
            assert selected_logits.shape == (k,self.vocab_size), \
