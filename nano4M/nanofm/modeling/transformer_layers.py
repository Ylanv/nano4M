...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        assert (
            dim % num_heads == 0
        ), "Dim of embeddings is not divisible by the number of heads"
    ...
    def forward(
        ...
        assert q.shape == (
            ...
            L,
            self.head_dim,
        ), f"Shape of Q is not correct should be ({B},{self.num_heads},{L},{self.head_dim})"
        assert k.shape == (
            B,
            self.num_heads,
            L,
            self.head_dim,
        ), f"Shape of K is not correct should be ({B},{self.num_heads},{L},{self.head_dim})"
        assert v.shape == (
            B,
            self.num_heads,
            L,
            self.head_dim,
        ), f"Shape of V is not correct should be ({B},{self.num_heads},{L},{self.head_dim})"
        ...
        if mask is not None:
            mask = rearrange(
                mask, "b n m -> b 1 n m"
            )  # Unsqueeze for multi-head attention
        ...
        assert attn.shape == (
            ...
            L,
            L,
        ), f"After softmax : {attn.shape} , should be ({B},{self.num_heads},{L},{L})"
        ...
        assert attn.shape == (
            ...
            L,
            D,
        ), f"attn @ V is not reshaped as orginal shape : ({B},{L},{D}) , Attn shape : {attn.shape}"
...
class CrossAttention(nn.Module):
    ...
    def forward(
        ...
        # Hint: Use the already defined self.scale
        attn = q @ k.transpose(-2, -1) * self.scale
        assert attn.shape == (B, self.num_heads, N, M), f"Att wrong shape: {attn.shape}"
        ...
        assert attn.shape == (
            ...
            N,
            C,
        ), f"attn @ V is not reshaped as orginal shape : ({B},{N},{C}) , Attn shape : {attn.shape}"
...
class Block(nn.Module):
    ...
    def __init__(
        ...
        self.attn = Attention(
            dim=dim, head_dim=head_dim, qkv_bias=use_bias, proj_bias=use_bias
        )
    ...
    def forward(
        ...
        assert (
            x.dim() == 3
        ), f"Expected input tensor x to have 3 dimensions (B, L, D), got {x.dim()}"
        B, L, D = x.shape
...
class DecoderBlock(nn.Module):
    ...
    def __init__(
        ...
        self.query_norm = LayerNorm(
            normalized_shape=dim, bias=use_bias
    ...
    def forward(
        ...
        xb = xa + self.cross_attn(
            self.query_norm(xa), self.context_norm(context), xa_mask
        )
...
class TransformerTrunk(nn.Module):
    ...
    def __init__(
        ...
        self.blocks = nn.ModuleList(
            [
                Block(
                    dim=dim, head_dim=head_dim, mlp_ratio=mlp_ratio, use_bias=use_bias
                )
